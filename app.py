...
class MQL5Compiler:
    ...
    def compile(self, mql5_code: str, ea_name: str = "PY2MQL5_Test") -> dict:
        ...
        # Compiled = .ex5 created (most reliable) OR zero errors in log
        # .ex5 is the ground truth — MetaEditor only creates it on success
        result["compiled"] = result["ex5_created"] or len(errors) == 0
    ...
    def _parse_log(self, log_path: str) -> tuple:
        ...
        for line in lines:
            ...
            # Skip success summary line:
            # ": information: result 0 errors, 0 warnings, 345 ms elapsed"
            if "information" in ll and "result" in ll and "elapsed" in ll:
                continue

            # Skip any line reporting 0 errors (MetaEditor success)
            if "0 errors" in ll:
                continue

            # Real errors have file(line,col) format
            if "error" in ll and "(" in ll and ")" in ll:
                clean = self._clean_log_line(line)
                if clean:
                    errors.append(clean)
            elif "warning" in ll and "(" in ll and ")" in ll:
                clean = self._clean_log_line(line)
                if clean:
...
class MT5StrategyTester:
    """
    MT5 Strategy Tester using MetaTrader5 Python API.
    No GUI automation needed — reads trade history directly.
    Windows only. pip install MetaTrader5
    
    Flow:
    1. Run EA in MT5 Strategy Tester manually (or via ini file)
    2. Read the XML/HTML report MT5 generates
    3. Extract first 5 trades: entry time, exit time, direction, PnL
    4. Compare with Python backtest signals
    """
    ...
    # ── APPROACH 1: Read existing MT5 report (simplest) ───────────
    def read_last_report(self, max_trades: int = 5) -> dict:
        """
        Read the most recent MT5 Strategy Tester HTML report.
        User runs the backtest manually in MT5, we read the result.
        Returns first max_trades trades with entry/exit/direction/PnL.
        """
        result = {
            "success": False,
            "trades":  [],
        ...
        if not report_path:
            result["error"] = (
                "No MT5 report found. Run a backtest in MT5 Strategy Tester first, "
                "then click this button."
            )
            return result

        result["report_path"] = report_path
        trades = self._parse_report(report_path, max_trades)

        if trades is not None:
            result["success"] = True
            result["trades"]  = trades
        else:
            result["error"] = "Could not parse MT5 report"

        return result

    # ── APPROACH 2: MT5 Python API (reads history directly) ────────
    def read_via_api(self, 
                     symbol:    str = "GBPUSD",
                     timeframe: str = "M15",
                     date_from: str = "2024.01.01",
                     date_to:   str = "2024.12.31",
                     max_trades: int = 5) -> dict:
        """
        Use MetaTrader5 Python API to read deal history.
        ...
        try:
            ...
            try:
                ...
                to_dt   = datetime.datetime(2024, 12, 31)
            ...
            if deals is None or len(deals) == 0:
                result["error"] = (
                    ...
                    "Run a backtest in Strategy Tester first, "
                    "then read via report file."
                )
                return result

            # Extract first max_trades trades
            trades = []
            for d in deals[:max_trades * 2]:  # get more to filter
                if d.type not in (0, 1):  # 0=buy, 1=sell
                    continue
                trades.append({
                    "entry_time": datetime.datetime.fromtimestamp(d.time).strftime("%Y.%m.%d %H:%M"),
                    "exit_time":  "",  # filled from paired deal
            ...
            result["error"] = f"MT5 API error: {e}"

        return result

    # ── APPROACH 3: Parse XML report from Strategy Tester ──────────
    def _parse_report(self, report_path: str, max_trades: int = 5) -> Optional[list]:
        """Parse MT5 HTML report and extract trade logs."""
        trades = []
        ...
        try:
            ...
            # MT5 report structure: find the trades table
            # Look for rows with buy/sell in them
            for row in soup.find_all("tr"):
                ...
                if "buy" not in row_text and "sell" not in row_text:
                    continue

                # Try to extract: time, type, lots, price, profit
                direction = "buy" if "buy" in row_text else "sell"
                ...
                for col in cols:
                    ...
                    if re.match(r"\d{4}\.\d{2}\.\d{2}", col_clean):
                        times.append(col_clean)
                    # Looks like a price or PnL
                    elif re.match(r"-?[\d]+\.[\d]+$", col_clean):
                        val = float(col_clean)
                        if 0.5 < abs(val) < 10:  # likely price for GBPUSD
                            price = col_clean
                        elif abs(val) < 10000:   # likely PnL
                            pnl = val

                trades.append({
                    "entry_time": times[0] if times else cols[0],
                    "exit_time":  times[1] if len(times) > 1 else "",
                    "direction":  direction,
                    "price":      price,
                    "pnl":        round(pnl, 2),
                })
            ...
            # Fallback: regex parse without bs4
            trades = self._parse_report_regex(report_path, max_trades)

        return trades if trades else None

    def _parse_report_regex(self, report_path: str, max_trades: int) -> list:
        """Fallback regex parser."""
        trades = []
        try:
            for enc in ["utf-8", "utf-16", "latin-1"]:
                try:
                    with open(report_path, encoding=enc, errors="ignore") as f:
                        html = f.read()
                    break
                except Exception:
                    continue
            ...
            for row in rows:
                ...
                if "buy" in row_text or "sell" in row_text:
                    direction = "buy" if "buy" in row_text else "sell"
                    times = [c for c in cells if re.match(r"\d{4}\.\d{2}\.\d{2}", c)]
                    pnl_vals = [float(c) for c in cells
                               if re.match(r"-?\d+\.\d+$", c) and abs(float(c)) < 10000]
                    trades.append({
                        "entry_time": times[0] if times else "",
                        "exit_time":  times[1] if len(times) > 1 else "",
                        "direction":  direction,
                        "price":      "",
                        "pnl":        round(pnl_vals[-1], 2) if pnl_vals else 0.0,
                    })
                    if len(trades) >= max_trades:
                        break
        except Exception:
    ...
    def compare_signals(python_signals: list, mt5_trades: list) -> dict:
        """Compare Python backtest signals vs MT5 trade log."""
        if not python_signals and not mt5_trades:
            return {"score": 100, "verified": True, "reason": "Both: 0 trades"}

        if not python_signals or not mt5_trades:
            return {
                "score": 0, "verified": False,
                "reason": f"Python: {len(python_signals)} | MT5: {len(mt5_trades)} trades"
            }

        py_dirs  = [s.get("direction", "") for s in python_signals]
        mt5_dirs = [t.get("direction", "") for t in mt5_trades]

        min_len  = min(len(py_dirs), len(mt5_dirs))
        matches  = sum(1 for a, b in zip(py_dirs[:min_len], mt5_dirs[:min_len]) if a == b)
        count_ok = len(python_signals) == len(mt5_trades)

        score = int((matches / min_len * 60) if min_len > 0 else 0)
        score += 40 if count_ok else 0

        return {
            "score":     score,
            "verified":  score >= 85,
            "badge":     "Verified" if score >= 85 else "Review needed",
...
with col_r:
    ...
    if go and code_in.strip():
        ...
        if result.mql5_code:
            st.session_state["last_mql5"]       = result.mql5_code
            st.session_state["last_ea_name"]    = ea_name
            st.session_state["last_confidence"] = result.confidence
            st.session_state["last_indicators"] = result.detected_indicators
            st.session_state["last_patterns"]   = result.detected_patterns
            st.session_state["last_errors"]     = result.errors
            st.session_state["last_warnings"]   = result.warnings
            st.session_state["last_notes"]      = result.notes
            st.session_state["last_ai_used"]    = result.ai_used
            st.session_state["conv_groq"]       = conv.groq_client
            st.session_state["compile_result"]  = None
        else:
            st.error("Conversion failed — check your Python code")

    elif go:
        st.warning("Paste your Python strategy first")

    if st.session_state.get("last_mql5"):
        mql5_code  = st.session_state["last_mql5"]
        ea_n       = st.session_state["last_ea_name"]
        conf       = st.session_state["last_confidence"]
        indicators = st.session_state["last_indicators"]
        ...
        ai_used    = st.session_state["last_ai_used"]
        groq_cl    = st.session_state.get("conv_groq")

        m1, m2, m3 = st.columns(3)
        cclass = "#00ff88" if conf >= 75 else "#f59e0b" if conf >= 50 else "#ef4444"
        with m1:
            st.markdown(f'''<div class="mcard"><h4>Confidence</h4>
<span style="font-size:1.5rem;font-weight:700;color:{cclass}">{conf}%</span></div>''', unsafe_allow_html=True)
        with m2:
            st.markdown(f'''<div class="mcard"><h4>Indicators</h4>
<span style="font-size:1.5rem;font-weight:700;color:#a78bfa">{len(indicators)}</span></div>''', unsafe_allow_html=True)
        with m3:
            ai_txt = "Groq" if ai_used else "Dict"
            ai_col = "#f59e0b" if ai_used else "#00ff88"
            st.markdown(f'''<div class="mcard"><h4>Engine</h4>
<span style="font-size:1.1rem;font-weight:700;color:{ai_col}">{ai_txt}</span></div>''', unsafe_allow_html=True)

        if indicators:
            tags = " ".join(f'<span class="tag tg">{i["name"].upper()}</span>' for i in indicators)
            st.markdown(tags, unsafe_allow_html=True)

        clean_pats = [p for p in patterns if p not in ("has_long","has_short")]
        if clean_pats:
            tags = " ".join(f'<span class="tag tp">{p.replace("_"," ")}</span>' for p in clean_pats)
            st.markdown(tags, unsafe_allow_html=True)

        for e in errors:
            st.markdown(f'<span class="tag tr">X {e}</span>', unsafe_allow_html=True)
        for w in warnings:
            st.markdown(f'<span class="tag ta">W {w}</span>', unsafe_allow_html=True)
        for n in notes:
            st.info(f"i {n}")
        ...
        # ── Get validation server URL from Streamlit secrets ─────
        try:
            server_url    = st.secrets.get("VALIDATION_SERVER_URL", "")
            server_secret = st.secrets.get("VALIDATION_API_SECRET", "")
        except Exception:
            server_url    = ""
            server_secret = ""

        if server_url:
            # ── CLOUD MODE: Call your Windows validation server ───
            col1, col2 = st.columns(2)

            with col1:
                if st.button("Compile + Backtest", use_container_width=True, key="validate_btn"):
                    with st.spinner("Sending to validation server..."):
                        try:
                            headers = {"x-api-secret": server_secret}
                            payload = {
                                "mql5_code":  mql5_code,
                                "ea_name":    ea_n,
                                "symbol":     "GBPUSD",
                            ...
                            if resp.status_code == 200:
                                st.session_state["validation_result"] = resp.json()
                            else:
                            ...
                            st.session_state["validation_result"] = {
                                "error": f"Could not reach validation server: {ex}"
                            }

            with col2:
                # Server status check
                try:
                    health = req_lib.get(f"{server_url}/health", timeout=5)
                    if health.status_code == 200:
                        h = health.json()
                        st.success("Server online")
                        st.caption(f"MetaEditor: {'✅' if h.get('metaeditor_found') else '❌'} | MT5: {'✅' if h.get('mt5_available') else '❌'}")
                    else:
                        st.warning("Server unreachable")
                except Exception:
                    st.warning("Validation server offline")
                    st.caption("Start validation_server.py on your Windows machine")
            ...
            if vr:
                if vr.get("error"):
                    ...
                    cr = vr.get("compile", {})
                    if cr.get("compiled"):
                        st.success(f"✅ Compiled clean — 0 errors · {cr.get('warning_count',0)} warnings")
                    else:
                        st.error(f"❌ Compile failed — {cr.get('error_count',0)} errors")
                        with st.expander("Compile errors"):
                            for e in cr.get("errors", []):
                                st.code(e, language=None)

                    # Backtest result
                    br = vr.get("backtest", {})
                    if br and br.get("success") and br.get("trades"):
                        st.markdown("**MT5 Trade Log — GBPUSD M15 · first 5 trades**")
                        import pandas as pd
                        df = pd.DataFrame(br["trades"])
                        show_cols = [c for c in ["entry_time","exit_time","direction","price","pnl"] if c in df.columns]
                        st.dataframe(df[show_cols], use_container_width=True)
                        if br.get("report_file"):
                            st.caption(f"Source: {br['report_file']}")
                    elif br and br.get("error"):
                        st.warning(f"Backtest: {br['error']}")

        else:
            # ── LOCAL MODE: No server configured ─────────────────
            import platform
                ...
                try:
                    ...
                    if status["ready"]:
                        if st.button("Compile in MetaEditor", use_container_width=True, key="compile_btn"):
                            ...
                            st.session_state["compile_result"] = cr

                        cr = st.session_state.get("compile_result")
                        if cr is not None:
                            if cr.get("compiled"):
                                st.success(f"✅ Compiled clean — 0 errors · {cr.get('warning_count',0)} warnings · attempt {cr.get('attempt',1)}/3")
                            else:
                                st.error(f"❌ {cr.get('error_count',0)} errors after {cr.get('attempt',1)} attempt(s)")
                                with st.expander("Compile errors"):
                                    for e in cr.get("errors",[]): st.code(e, language=None)
                    else:
                        st.warning("MetaEditor not found")
                        mp = st.text_input("MetaEditor path manually", placeholder=r"C:\Program Files\MetaTrader 5\metaeditor64.exe", key="mp")
                        if mp and st.button("Compile", key="cm"):
                            with st.spinner("Compiling..."):
                                cr = MQL5Compiler(metaeditor_path=mp).compile_and_fix(mql5_code, ea_n, max_attempts=3)
                            st.session_state["compile_result"] = cr
                except Exception as ex:
                    st.error(f"Compiler error: {ex}")

                st.info(
                    "To enable full validation for users: "
                    "run validation_server.py on this machine, "
                    "expose via ngrok, add URL to Streamlit secrets."
                )
            else:
                st.info(
                    "Validation server not configured. "
                    "Run validation_server.py on your Windows machine "
                    "and add VALIDATION_SERVER_URL to Streamlit secrets."
                )
        ...
        <div style="height:340px;display:flex;flex-direction:column;align-items:center;
                    ...
                    color:#1e3a5f;font-family:monospace;font-size:.82rem;text-align:center;gap:1rem">
